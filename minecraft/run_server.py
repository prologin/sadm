...
class MCWatchLog:
    ...
    def handle(self, line):
        ...
        # FIXME(Zopieux): ip_to_nick does not exist (yet)
        expected_nick = prologin.presenced.ip_to_nick(ip)
...
def install_server_properties():
    with open(os.path.join(SERVER_CFG['server']['working_dir'], 'server.properties'), 'w', encoding='ascii') as f:
        f.write("#Minecraft server properties\n")
        f.write("#Generated by Prolocraft\n")
        for key, value in SERVER_CFG['properties'].items():
            f.write("%s=%s\n" % (key.replace('_', '-'), value))

    logging.info("Wrote server.properties")
...
if __name__ == '__main__':
    ...
    time.sleep(1)  # just to be sure server.log is created
